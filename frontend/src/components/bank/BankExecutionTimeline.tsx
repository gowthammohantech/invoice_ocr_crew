...
const STAGES = [
  ...
  },
  {
    ...
    desc: "Parse transactions into JSON",
  ...
  },
  {
    ...
    desc: "Reconcile balances & totals",
  ...
  },
  {
    ...
    activeColor: "border-teal-400 bg-teal-50 shadow-sm shadow-teal-100",
...
export default function BankExecutionTimeline({ events, isStreaming, isDone, fileStem, isPending }: Props) {
        ...
        <div className="flex items-stretch gap-2">
          {STAGES.map((stage, i) => {
            ...
            return (
              <div key={stage.id} className="flex items-center gap-2 flex-1 min-w-0">
                <div className="flex-1 min-w-0">
                  <StageCard stage={stage} state={state} tool={tool} isStreaming={isStreaming} />
                </div>
                {i < STAGES.length - 1 && (
                  <span className="text-slate-300 text-lg flex-shrink-0 select-none">›</span>
                )}
        ...
        <div className={cn(
          ...
          {allDone ? (
            <>
              ...
              {resolvedStem && (
                <Link
                  href={`/bank-statements/${resolvedStem}`}
